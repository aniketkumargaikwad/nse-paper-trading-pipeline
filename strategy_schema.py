"""Loader + validator for `strategies.yaml`.

The strategy file is the ONLY place trading logic is defined, so this module
is deliberately strict: any typo, unknown indicator, or missing field raises
:class:`StrategyConfigError` with a message that points at the exact location
(e.g. ``strategies[0].entry.all[1].operator``) and says how to fix it.

This module is pure (no network, no database) so it is fully unit-testable
and safe to run anywhere:

    python strategy_schema.py                # validates ./strategies.yaml
    python strategy_schema.py path/to.yaml   # validates a specific file

Design notes
------------
* Hand-rolled validation instead of jsonschema/pydantic: the error messages
  those libraries produce are cryptic for a non-expert, and our schema is
  small enough that explicit checks stay readable.
* The dataclasses returned here are the canonical in-memory representation of
  a strategy; the signal engine (component 4) consumes them directly, so the
  engine never touches raw YAML.
"""
...
# ---------------------------------------------------------------------------
# Vocabulary: the single source of truth for what the YAML may contain.
# ---------------------------------------------------------------------------
