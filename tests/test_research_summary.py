...
def test_top_and_bottom_are_capped_and_ordered():
    ...
    assert got.top[0]["excess_vs_hold_pct"] >= got.top[-1]["excess_vs_hold_pct"]
    assert got.bottom[0]["excess_vs_hold_pct"] <= got.bottom[-1]["excess_vs_hold_pct"]


# --- measuring against the market, not against zero --------------------------


def test_a_row_says_what_it_returned_and_what_holding_returned():
    """Three +5% trades compound to +15.76%, against a market that rose 2%."""
    got = build_summary([winner("A", "day", n=3, pct=5.0, hold=2.0)],
                        FREE, window_days_for=lambda r: 365)
    row = got.top[0]
    assert row["return_pct"] == pytest.approx(15.76, abs=0.01)
    assert row["hold_return_pct"] == pytest.approx(2.0)
    assert row["excess_vs_hold_pct"] == pytest.approx(13.76, abs=0.01)


def test_top_is_ordered_by_excess_over_holding_not_by_raw_profit():
    """The whole point: a big number in a bigger market is not an edge."""
    beta = winner("BETA", "day", n=3, pct=10.0, hold=100.0)     # +33%, market +100%
    edge = winner("EDGE", "day", n=3, pct=2.0, hold=-10.0)      # +6%, market -10%
    got = build_summary([beta, edge], FREE, window_days_for=lambda r: 365)
    assert got.top[0]["symbol"] == "EDGE"
    assert got.top[0]["net_pnl"] < got.top[1]["net_pnl"]        # and it earned less


def test_how_many_combinations_beat_simply_holding_is_counted():
    beta = winner("BETA", "day", n=3, pct=10.0, hold=100.0)
    edge = winner("EDGE", "day", n=3, pct=2.0, hold=-10.0)
    got = build_summary([beta, edge], FREE, window_days_for=lambda r: 365)
    assert got.combos_tested == 2
    assert got.combos_profitable == 2       # both made money
    assert got.combos_beating_hold == 1     # only one made more than the market


def test_a_timeframe_says_what_share_of_it_beat_holding():
    got = build_summary(
        [winner("A", "day", n=3, pct=10.0, hold=100.0),
         winner("B", "day", n=3, pct=2.0, hold=-10.0)],
        FREE, window_days_for=lambda r: 365,
    )
    assert got.per_timeframe[0]["symbols_beating_hold_pct"] == pytest.approx(50.0)


def test_a_combination_with_no_hold_return_ranks_last():
    """Without a benchmark a result cannot be judged, so it must not lead."""
    blind = winner("BLIND", "day", n=3, pct=50.0, hold=None)
    modest = winner("MODEST", "day", n=3, pct=1.0, hold=0.0)
    got = build_summary([blind, modest], FREE, window_days_for=lambda r: 365)
    assert got.top[0]["symbol"] == "MODEST"
    assert got.top[-1]["excess_vs_hold_pct"] is None
